...
char* indexes(const char* path)
{
    // create possible php index from path
    char* index_php = malloc(sizeof(char) * (strlen(path) + strlen("index.php") + 2));
    strcpy(index_php, path);
    strcat(index_php, "index.php");
    
    if(access(index_php, F_OK) == 0)
        return index_php;
    // no index, therefore free index_php
    free(index_php);
    
    // create possible html index from path
    char* index_html = malloc(sizeof(char) * (strlen(path) + strlen("index.html") + 2));
    strcpy(index_html, path);
    strcat(index_html, "index.html");
    
    if (access(index_html, F_OK) == 0)
        return index_php;
    // no index, therefore free index_html
    free(index_html);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    {
        // increase buffer by 1 BYTE in each iteration
        buffer = realloc(buffer, sizeof(char) + (*length));
        // buffer stores subsequent value
        *(buffer + (*length)) = i;
        // length is incremented
        (*length)++;
    }
    
    // content points to data stored in buffer
    *content = buffer;
...
{
    // looks for ending 
    char* endingPtr = strrchr(path, '.');
    
    // return MIME type
    if (endingPtr != NULL)
    {
        if (strcasecmp(endingPtr, ".css") == 0)
            return "text/css";
        else if (strcasecmp(endingPtr, ".html") == 0)
            return "text/html";
        else if (strcasecmp(endingPtr, ".gif") == 0)
            return "image/gif";
        else if (strcasecmp(endingPtr, ".ico") == 0)
            return "image/x-icon";
        else if (strcasecmp(endingPtr, ".jpg") == 0)
            return "image/jpeg";
        else if (strcasecmp(endingPtr, ".js") == 0)
            return "text/javascript";
        else if (strcasecmp(endingPtr, ".php") == 0)
            return "text/x-php";
        else if (strcasecmp(endingPtr, ".png") == 0)
            return "image/png";
    }
    
    // if MIME is incompatible
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // locate start of request target
    char* reqtarPtr = strchr(line, ' ') + 1;
    
    // request target must start with '/'
    if (strncmp(reqtarPtr, "/", 1) != 0)
    {
        error(501);
        return false;
    }
    
    //locate end of request target by looking at next whitespace
    char* reqtarEndPtr = strchr(reqtarPtr, ' ') - 1;
    
    // request target must not contain '\"'
    if (strchr(reqtarPtr, '\"') != NULL && strchr(reqtarPtr, '\"') <= reqtarEndPtr)
    {
        error(400);
        return false;
    }
    ...
    // also checks for only one whitespace separating HTTP version and request target
    if (strncmp(reqtarEndPtr + 1, " HTTP/1.1", 9) != 0)
    {
        error(505);
        return false;
    }
    
    // iterate until ? or whitespace is reached
    int i;
    for (i = 0; reqtarPtr[i] != '?' && reqtarPtr[i] != ' '; i++)
        abs_path[i] = reqtarPtr[i];
    abs_path[i] = '\0';
    
    // if query exists, store data after '?'
    if (reqtarPtr[i] == '?' && reqtarPtr[i + 1] != ' ')
    {
        i++;
        int j;
        for (j = 0; reqtarPtr[i] != ' '; i++, j++)
            query[j] = reqtarPtr[i];
        query[j] = '\0';
    }
    else
